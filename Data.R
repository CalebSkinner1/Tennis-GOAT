...
players <- read_csv(here("data/atp_players.csv")) %>%
...
rankings <- read_csv(files_rankings, id = "path") %>%
  ...
  left_join(players, by = join_by(player_id)) %>%
...
players_results <- matches %>%
  ...
  mutate(round = factor(round, levels = c("R128", "R64", "R32", "R16", "QF", "SF", "F"))) %>% 
  arrange(date, round) %>%
  left_join(players, join_by(player_id, name == player)) %>%
  mutate(dob = ymd(dob))
